...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/05 19:03:30 by austunso          #+#    #+#             */
/*   Updated: 2024/12/06 14:00:13 by austunso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strjoin(char *buffer, char *temp_buffer)
{
	...
	if (!buffer || !temp_buffer)
		return (NULL);
	new_buffer = malloc(sizeof(char) * (ft_strlen(buffer)
				+ ft_strlen(temp_buffer) + 1));
	i = -1;
	j = 0;
	while (buffer[++i])
